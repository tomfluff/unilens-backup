...
export function computeExpandedFontPx(originalPx: number, level: TextAdjustLevel = a11ySettings.bodyTextExpandLevel): number {
...
function applyExpand(el: HTMLElement, originalPx: number, level: TextAdjustLevel) {
  const targetPx = computeExpandedFontPx(originalPx, level)
  el.setAttribute(BODY_EXPAND_ATTR, 'true')
  el.setAttribute(BODY_EXPAND_ORIGINAL_ATTR, String(Math.round(originalPx * 100) / 100))
...
export function scanBodyText(root: Element = document.body): BodyTextScanResult {
  ...
  for (const el of nodes) {
    ...
    if (px >= LARGE_TEXT_PX) {
      if (el.getAttribute(BODY_EXPAND_ATTR) === 'true') restoreElement(el)
      continue
    }
    ...
    if (
      ...
      el.getAttribute(BODY_EXPAND_ORIGINAL_ATTR) === String(Math.round(px * 100) / 100)
    ) {
      continue
    }
...
export function clearBodyTextExpand() {
  document.querySelectorAll(`[${BODY_EXPAND_ATTR}="true"]`).forEach((node) => {
...
function applyEnabledState() {
  ...
  if (level > 0 && bodyTextExpandConfig(level)) {
    ...
    document.documentElement.setAttribute(BODY_EXPAND_LEVEL_ATTR, String(level))
