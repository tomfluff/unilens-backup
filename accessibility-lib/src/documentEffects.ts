...
export function ensurePageContentRoot(): HTMLDivElement | null {
  ...
  let root = document.getElementById(PAGE_CONTENT_ROOT_ID) as HTMLDivElement | null
  if (root) return root
...
function ensurePositionedOverlayObserver(pageRoot: HTMLElement | null, active: boolean) {
  ...
  overlayMarkerObserver = new MutationObserver(() => schedulePositionedOverlaySync(pageRoot))
...
export function applyA11yToDocument() {
  ...
  html.style.setProperty(HTML_STYLE_LETTER_SPACING, LETTER_SPACING_EM[s.letterSpacing])
  html.style.setProperty(HTML_STYLE_WORD_SPACING, WORD_SPACING_EM[s.letterSpacing])
  html.style.setProperty(HTML_STYLE_SATURATE, String(SATURATION_FILTER[s.saturation]))
  html.style.setProperty(HTML_STYLE_CONTRAST, String(CONTRAST_FILTER[s.contrast]))
