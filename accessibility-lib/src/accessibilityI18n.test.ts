...
import { bodyExpandLevelLabels, smallBoostLevelLabels } from './accessibilityPanelOptions'
import { bodyTextExpandHint, smallTextBoostHint, type TextAdjustLevel } from './textAdjustLevels'

/** Recursively extracts a dictionary's shape (key order, value kind, function arity). */
function shapeOf(value: unknown, path = ''): string[] {
  if (typeof value === 'function') return [`${path}: fn/${(value as (...a: unknown[]) => unknown).length}`]
...
describe('language dictionary coverage', () => {
  it('every language has the same keys and the same function arities', () => {
    ...
    A11Y_LANGS.forEach((lang) => {
      expect(shapeOf(A11Y_MESSAGES[lang]), `${lang} dictionary`).toStrictEqual(reference)
    })
  })
  ...
  it('level labels: only the name changes per language, the hint stays shared', () => {
    ...
    A11Y_LANGS.forEach((lang) => {
      ...
      levels.forEach((level) => {
        ...
        expect(smallBoostLevelLabels()[level][1]).toBe(smallTextBoostHint(level))
      })
    })
