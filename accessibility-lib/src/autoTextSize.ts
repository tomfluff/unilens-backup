 ...
 */
import { getSettings, onSettingsChange, saveSettings, settings } from './settings'
...
function clampScale(n: number): number {
  return Math.min(settings.autoTextMaxScale, Math.max(1, Math.round(n * 100) / 100))
}
...
export function analyzePageText(): TextAnalysis {
  ...
  for (const el of document.body.querySelectorAll('*')) {
    ...
    if (TEXT_TAGS.has(el.tagName) === false && !el.textContent?.trim()) continue
  ...
  else if (smallTextRatio > 0.4) contentScale = targetMin / Math.max(minPx, 10)
  ...
  else if (settings.autoTextMode === 'combined') suggestedScale = clampScale(Math.max(vpScale, contentScale))
...
function applyScaleToPage(scale: number) {
  ...
  document.documentElement.style.setProperty('--unilens-text-scale', String(scale))
  document.documentElement.style.setProperty('--unilens-text-line', scale > 1.1 ? '1.08' : '1')
}
...
export function getEffectiveChatFontSize(): number {
  if (!settings.autoTextSize || settings.autoTextMode === 'off') return settings.chatFontSize
...
export function runAutoTextAction(action: 'analyze' | 'apply' | 'reset' | 'bumpUp' | 'bumpDown'): TextAnalysis {
  if (action === 'reset') {
  ...
  if (action === 'bumpUp' || action === 'bumpDown') {
    ...
    const next = clampScale(appliedScale + (action === 'bumpUp' ? 0.1 : -0.1))
...
function onResize() {
  ...
  resizeTimer = window.setTimeout(() => {
    if (settings.autoTextSize && settings.autoTextMode !== 'off') refreshAutoTextSize()
  }, 250)
}
