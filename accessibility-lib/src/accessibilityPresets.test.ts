...
describe('accessibility presets', () => {
  ...
  it('letter-spacing alone does not set legacy font-active flag', () => {
    ...
    applyA11yToDocument()
    expect(stub.html.getAttribute('data-unilens-a11y-font-active')).toBeNull()
    expect(stub.html.getAttribute('data-unilens-a11y-font-size')).toBe('standard')
    expect(stub.html.getAttribute('data-unilens-a11y-line-height')).toBe('standard')
    expect(stub.html.getAttribute('data-unilens-a11y-letter-spacing')).toBe('wide')
  })
  ...
  it('applyA11yToDocument writes prefixed html attributes and css vars', () => {
    ...
    expect(stub.html.style.getPropertyValue(HTML_STYLE_SATURATE)).toBe(String(SATURATION_FILTER.soft))
    expect(stub.html.style.getPropertyValue(HTML_STYLE_LETTER_SPACING)).toBe('0.12em')
