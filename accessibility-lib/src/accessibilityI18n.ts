...
const ja = {
  ...
  openPanelWithCount: (count: number) => `表示調整パネルを開く（${count} 件適用中）`,
  toggleLabel: '表示調整',
  tabsLabel: '表示調整のカテゴリ',
  tabWithCount: (label: string, count: number) => `${label}（${count} 件適用中）`,
  chipsLabel: '適用中',
  ...
  // Language switcher
  languageTitle: '表示言語',
  languageDesc: 'この表示調整パネルの言語を切り替えます。ページ本文は翻訳されません。',
  ...
  presetsTitle: 'よく使う設定',
  presetsDesc: '用途別の組み合わせをワンタップで適用します。もう一度押すとオフになります。',
  presetsGroup: 'プリセット',
  ...
  cuesDesc: '色だけに頼らず、リンクや今の位置が分かるようにします。動きを止めることもできます。',
  ...
  elementHighlightHintOn: 'フォーカスしていなくても操作できる場所が分かります',
  elementHighlightHintOff: '注意がそれやすいときに、操作対象を見失いにくくします',
  ...
  pageTextDesc: 'サイト全体の文字サイズ・書体・行間・文字間隔をまとめて変更します。',
  fontSizeGroup: '文字サイズ',
  ...
  smallBoostDesc: '注釈やキャプションなど 16px 未満の文字だけを、読める大きさまで引き上げます。',
  ...
  selectionDesc: 'ページ上でドラッグして選んだ範囲だけ、その場で大きくできます。',
  selectionQuoted: (text: string, charCount: number) => `「${text}」${charCount} 文字`,
  selectionSticky: '保持中',
  ...
  speakDesc: 'ページ上でドラッグして選んだ文章を音声で読み上げます。読んでいる場所は色で追えます。',
  ...
  speakSpeakingNow: (charCount: number) => `${charCount} 文字を読み上げています`,
...
const en: A11yMessages = {
  ...
  resetConfirm: 'To confirm, press again to reset every setting to its default',
  ...
  presetsDesc: 'One-tap combinations for common needs. Press again to turn off.',
  ...
  cuesDesc: 'Make links and your current position clearer without relying on color alone. You can also stop motion.',
  linkUnderlineSwitch: 'Always underline links',
  linkUnderlineHintOn: 'Links stay recognizable even when colors are hard to tell apart',
  linkUnderlineHintOff: 'Especially helpful for color vision support and low vision',
  ...
  linkBackgroundHintOn: 'Recognizable as a shape, not just an underline or color',
  linkBackgroundHintOff: 'Useful when underlines are hard to spot against the background',
  ...
  elementHighlightHintOn: 'Shows what you can interact with, even without focusing it first',
  elementHighlightHintOff: 'Helps you keep track of interactive targets when attention drifts easily',
  ...
  pageTextDesc: 'Changes the text size, typeface, line spacing and letter spacing across the whole site.',
  ...
  typefaceUdAria: 'Typeface: UD font, with letters that are easier to tell apart',
  ...
  bodyExpandNote: (expanded, scanned) => `${expanded} places enlarged (${scanned} checked)`,
  rescan: 'Scan again',
  rescanBodyAria: 'Scan the page again and enlarge body text',
  rescanBodyResult: (expanded, scanned) => `${expanded} places enlarged (${scanned} checked)`,
  ...
  smallBoostAnnounce: (name, boosted) => `Small text boost set to ${name}. ${boosted} places raised`,
  smallBoostNote: (boosted, scanned) => `${boosted} places raised (${scanned} checked)`,
  rescanSmallAria: 'Scan the page again and raise small text',
  rescanSmallResult: (boosted, scanned) => `${boosted} places raised (${scanned} checked)`,

  selectionTitle: 'Enlarge selected text',
  selectionDesc: 'Drag to select text on the page, then enlarge just that part on the spot.',
  ...
  speakHighlightHintOn: 'Marks the passage and the current word in a color distinct from the selection',
  speakHighlightHintOff: 'Useful when you want to follow along while listening',
  ...
  autoModeAnnounce: (label, scale) => `Decision method set to ${label}. Scale ×${scale}`,
  autoMinGroup: 'Target minimum size',
  autoMinAria: (px) => `Target minimum size: ${px} pixels`,
  autoMinAnnounce: (px, scale) => `Target minimum size set to ${px} pixels. Scale ×${scale}`,
  ...
  selWhitespaceOnly: 'A selection containing only whitespace cannot be changed',
  selApplied: (label, px) => `Selected text set to ${label} (${px}px). The selection is kept`,
  ...
  selCleared: 'The size change on the selection has been undone. The selection is kept',
}
...
export function languageDescByLang(): { lang: A11yLang; text: string }[] {
  return A11Y_LANGS.map((lang) => ({ lang, text: MESSAGES[lang].languageDesc }))
}
