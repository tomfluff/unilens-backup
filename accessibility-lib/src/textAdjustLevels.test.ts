...
describe('computeBodyTextTargetPx', () => {
  ...
  it('gets larger as the level increases', () => {
    const sizes = [1, 2, 3].map((level) => computeBodyTextTargetPx(20, level as TextAdjustLevel))
  ...
  it('never boosts below the configured floor', () => {
    ;[1, 2, 3].forEach((level) => {
      ...
      for (let px = 6; px < 16; px += 1) {
        expect(computeSmallTextTargetPx(px, level as TextAdjustLevel)).toBeGreaterThanOrEqual(cfg.minPx)
      }
    })
  })
})
...
describe('deriving the level labels', () => {
  ...
  it('the hint is digits and symbols only, no language-dependent characters', () => {
    ;[1, 2, 3].forEach((level) => {
      expect(bodyTextExpandHint(level as TextAdjustLevel)).toMatch(/^[\d.]+×$/)
      expect(smallTextBoostHint(level as TextAdjustLevel)).toMatch(/^\d+px\+$/)
    })
  })
})
