 ...
 */
import { FONT_SCALE_LEVELS, levelToScale, scaleToLevel, type FontScaleLevel } from './fontScales'
...
function rangeRootElement(range: Range): Element | null {
  ...
  return root.nodeType === Node.TEXT_NODE ? root.parentElement : (root as Element)
}
...
function findResizeWrapper(node: Node | null): HTMLElement | null {
  ...
  while (cur) {
    if (cur instanceof HTMLElement && cur.hasAttribute(WRAP_ATTR)) return cur
    cur = cur.parentNode
  }
  return null
}
...
function getBaseFontSizePx(range: Range): number {
  const wrapper = findResizeWrapper(range.startContainer) ?? findResizeWrapper(range.endContainer)
  if (wrapper) {
    const stored = parseFloat(wrapper.getAttribute('data-unilens-font-base') ?? '')
...
export function pxToLevel(px: number, basePx?: number): SelectionFontLevel | null {
  const base = basePx ?? 16
...
function readWrapperState(wrapper: HTMLElement) {
  const basePx = parseFloat(wrapper.getAttribute('data-unilens-font-base') ?? '')
  const scale = parseFloat(wrapper.getAttribute('data-unilens-font-scale') ?? '')
  const px = parseFloat(wrapper.getAttribute('data-unilens-font-px') ?? wrapper.style.fontSize)
  return {
...
export function getSelectionInfo(): SelectionInfo {
  ...
  if (sel && sel.rangeCount > 0 && !sel.isCollapsed) {
    ...
    if (!isUniLensRange(range)) {
      ...
      if (text) {
        const wrapper = findResizeWrapper(sel.anchorNode) ?? findResizeWrapper(sel.focusNode)
        if (wrapper) {
          ...
          return {
            ...
            appliedLevel: scale != null ? scaleToLevel(scale) : px != null && basePx ? pxToLevel(px, basePx) : null,
            appliedPx: px,
            appliedScale: scale,
            sticky,
          }
  ...
  if (activeWrapper?.isConnected) {
    const text = activeWrapper.textContent?.replace(/\s+/g, ' ').trim() ?? ''
...
function wrapRange(range: Range, basePx: number, scale: number, level: SelectionFontLevel): HTMLElement {
  ...
  if (existing && range.intersectsNode(existing)) {
    ...
    existing.setAttribute('data-unilens-font-base', String(Math.round(basePx)))
...
export function applySelectionFontLevel(level: SelectionFontLevel): ApplySelectionResult {
...
export function clearSelectionFontSize(): ApplySelectionResult {
  const wrapper =
    activeWrapper?.isConnected ? activeWrapper :
    (() => {
      const sel = window.getSelection()
      return (sel && findResizeWrapper(sel.anchorNode)) ?? (sel && findResizeWrapper(sel.focusNode)) ?? null
    })()
...
export function initSelectionTextSize() {
  ...
  onSelectionChangeHandler = () => {
    ...
    if (!sel.isCollapsed) {
      ...
      else if (!activeWrapper?.contains(range.commonAncestorContainer)) activeWrapper = null
    }
...
export function destroySelectionTextSize() {
  ...
  if (onSelectionChangeHandler) document.removeEventListener('selectionchange', onSelectionChangeHandler)
