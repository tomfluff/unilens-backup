...
import { getAppliedTextScale, runAutoTextAction, type TextAnalysis } from './autoTextSize'
...
import { TABS, bodyExpandLevelLabels, smallBoostLevelLabels, tabLabel, type PanelTab } from './accessibilityPanelOptions'
import { ToolsTab, TextTab, VisualTab } from './accessibilityPanelTabs'
import { Button, Icon, LiveRegion, ROOT_ID, announce, useExternalSignal } from './accessibilityPanelUI'
...
function syncOverlayIsolation() {
  ...
  if (!root) return
  if (a11ySettings.reduceMotion) root.setAttribute(REDUCE_MOTION_ATTR, 'true')
...
function activeAdjustments(): Adjustment[] {
  ...
  const add = (tab: PanelTab, key: string, label: string, clear: () => void) => list.push({ tab, key, label, clear })

  if (s.theme !== 'standard') {
    add('text', 'theme', t().theme[s.theme], () => patchA11ySettings({ theme: 'standard' }))
  }
  if (s.saturation !== 'standard') {
    add('text', 'saturation', t().adjSaturation(t().saturation[s.saturation]), () =>
      patchA11ySettings({ saturation: 'standard' }),
    )
  }
  if (s.contrast !== 'standard') {
    add('text', 'contrast', t().adjContrast(t().contrast[s.contrast]), () => patchA11ySettings({ contrast: 'standard' }))
  }
  if (s.fontSize !== 'standard') {
    add('text', 'fontSize', t().adjFontSize(t().fontSize[s.fontSize]), () => patchA11ySettings({ fontSize: 'standard' }))
  }
  if (s.fontFamily === 'ud') {
    add('text', 'fontFamily', t().adjUdFont, () => patchA11ySettings({ fontFamily: 'standard' }))
  }
  if (s.fontFamily === 'custom' && s.customFontFamily) {
    add('text', 'fontFamily', t().adjCustomFont(s.customFontFamily), () =>
      patchA11ySettings({ fontFamily: 'standard', customFontFamily: null }),
    )
  }
  if (s.lineHeight === 'wide') {
    add('text', 'lineHeight', t().adjLineHeight, () => patchA11ySettings({ lineHeight: 'standard' }))
  }
  if (s.letterSpacing === 'wide') {
    add('text', 'letterSpacing', t().adjLetterSpacing, () => patchA11ySettings({ letterSpacing: 'standard' }))
  }
  if (s.linkUnderline) {
    add('visual', 'linkUnderline', t().adjLinkUnderline, () => patchA11ySettings({ linkUnderline: false }))
  }
  if (s.linkBackground) {
    add('visual', 'linkBackground', t().adjLinkBackground, () => patchA11ySettings({ linkBackground: false }))
  }
  if (s.linkBorder) {
    add('visual', 'linkBorder', t().adjLinkBorder, () => patchA11ySettings({ linkBorder: false }))
  }
  if (s.imageBorder) {
    add('visual', 'imageBorder', t().adjImageBorder, () => patchA11ySettings({ imageBorder: false }))
  }
  if (s.elementHighlight) {
    add('visual', 'elementHighlight', t().adjElementHighlight, () => patchA11ySettings({ elementHighlight: false }))
  }
  if (s.focusEnhance) {
    add('visual', 'focusEnhance', t().adjFocusEnhance, () => patchA11ySettings({ focusEnhance: false }))
  }
  if (s.reduceMotion) {
    add('visual', 'reduceMotion', t().adjReduceMotion, () => patchA11ySettings({ reduceMotion: false }))
  }
  if (s.bodyTextExpandLevel > 0) {
    const name = bodyExpandLevelLabels()[s.bodyTextExpandLevel][0]
    add('text', 'bodyTextExpandLevel', t().adjBodyExpand(name), () => patchA11ySettings({ bodyTextExpandLevel: 0 }))
  }
  if (s.smallTextBoostLevel > 0) {
    const name = smallBoostLevelLabels()[s.smallTextBoostLevel][0]
    add('text', 'smallTextBoostLevel', t().adjSmallBoost(name), () => patchA11ySettings({ smallTextBoostLevel: 0 }))
  }
  if (settings.autoTextSize && settings.autoTextMode !== 'off') {
    add('tools', 'autoTextSize', t().adjAutoSize(getAppliedTextScale().toFixed(2)), () => {
...
function TabBar({
  ...
  btnRefs: React_MutableRefObject<Partial<Record<PanelTab, HTMLButtonElement | null>>>
}) {
  return (
    <ul className="unilens-a11y-tabs" role="tablist" aria-label={t().tabsLabel}>
      {TABS.map((tab, index) => {
...
function Footer({ hasAdjustments, resetArmed, onResetPress }: { hasAdjustments: boolean; resetArmed: boolean; onResetPress: () => void }) {
  return (
    <div className="unilens-a11y-footer">
      ...
      <p className="unilens-a11y-hint">
        <span className="unilens-a11y-kbd">←</span> <span className="unilens-a11y-kbd">→</span>
        {` ${t().hintTabs} · `}
...
function AccessibilityPanelApp() {
  ...
  const [activeTab, setActiveTab] = useState<PanelTab>(() => restoreActiveTab())
  const [openedTabs, setOpenedTabs] = useState<Set<PanelTab>>(() => new Set([restoreActiveTab()]))
  const [resetArmed, setResetArmed] = useState(false)
  const [lastAnalysis, setLastAnalysis] = useState<TextAnalysis | null>(null)
  const [lastSelectionMessage, setLastSelectionMessage] = useState<{ text: string; ok: boolean } | null>(null)

  const toggleBtnRef = useRef<HTMLButtonElement | null>(null)
  const tabBtnRefs = useRef<Partial<Record<PanelTab, HTMLButtonElement | null>>>({})
  ...
  useEffect(() => {
    document.getElementById(ROOT_ID)?.classList.toggle(A11Y_OPEN_CLASS, panelOpen)
  }, [panelOpen])
  ...
  useEffect(() => {
    ...
    function onDblClick(e: globalThis.MouseEvent) {
      const root = document.getElementById(ROOT_ID)
      const insideWidget = e.target instanceof Node && root?.contains(e.target) === true
  ...
  function closePanel() {
    const hadFocusInside = document.getElementById(ROOT_ID)?.contains(document.activeElement) === true
    setPanelOpen(false)
    setResetArmed(false)
    window.clearTimeout(resetTimerRef.current)
  ...
  return (
    <>
      ...
      >
        ...
        <span className="unilens-a11y-toggle-text">{t().toggleLabel}</span>
      </button>
      ...
      >
        ...
        <TabBar activeTab={activeTab} onSwitch={switchTab} btnRefs={tabBtnRefs} />
        ...
        <div className="unilens-a11y-body" ref={bodyRef}>
          {TABS.map(({ id }) => (
            <div
              key={id}
              id={`da-tabpanel-${id}`}
              className={id === activeTab ? 'da-tabpanel unilens-a11y-tabpanel-active' : 'da-tabpanel'}
              role="tabpanel"
              ...
              {openedTabs.has(id) &&
                (id === 'visual' ? (
                  ...
                  <ToolsTab
                    ...
                    setLastSelectionMessage={setLastSelectionMessage}
                  />
                ))}
            </div>
          ))}
        ...
        <Footer hasAdjustments={adjustments.length > 0} resetArmed={resetArmed} onResetPress={handleResetPress} />
      </div>
...
export function destroyAccessibilityPanel() {
  ...
  if (panelDomRoot && panelAltPointerDown) {
    panelDomRoot.removeEventListener('mousedown', panelAltPointerDown, true)
