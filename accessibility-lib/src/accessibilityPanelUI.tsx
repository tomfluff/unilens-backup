 ...
 */
export function useExternalSignal(subscribe: (cb: () => void) => () => void): void {
...
const ICON_PATHS: Record<IconName, string> = {
  ...
  access:
    '<circle cx="12" cy="4" r="1.8"/><path d="M4.5 8.4 12 9.8l7.5-1.4"/><path d="m9.2 21 1.5-7.2M14.8 21l-1.5-7.2"/><path d="M9.6 13.8h4.8"/>',
  selection: '<path d="M9 4h6M9 20h6M12 4v16"/><path d="M4 8V6a2 2 0 0 1 2-2h1M20 8V6a2 2 0 0 0-2-2h-1M4 16v2a2 2 0 0 0 2 2h1M20 16v2a2 2 0 0 1-2 2h-1"/>',
  ...
  globe:
    '<circle cx="12" cy="12" r="9"/><path d="M3.2 9h17.6M3.2 15h17.6"/><path d="M12 3c2.3 2.5 3.5 5.6 3.5 9s-1.2 6.5-3.5 9c-2.3-2.5-3.5-5.6-3.5-9s1.2-6.5 3.5-9Z"/>',
  ...
  speaker: '<path d="M11 5 6.5 8.8H3v6.4h3.5L11 19z"/><path d="M15 9.4a3.6 3.6 0 0 1 0 5.2"/><path d="M17.8 6.6a7.6 7.6 0 0 1 0 10.8"/>',
...
function rovingKeyDown(e: KeyboardEvent, index: number, count: number, moveTo: (index: number) => void) {
  let next = -1
...
export function Button({ label, iconName, variant = 'secondary', ariaLabel, focusKey, full, disabled, onClick }: ButtonProps) {
  ...
  return (
    <section className="unilens-a11y-card">
      <div className="unilens-a11y-card-head">
        ...
        {badge && <span className="unilens-a11y-card-badge">{badge}</span>}
      </div>
...
export function ChoiceGroup<T extends string>({
  ...
  return (
    <fieldset className="unilens-a11y-group">
      ...
      <div className="unilens-a11y-choices" role="radiogroup" aria-label={label} style={{ '--unilens-a11y-panel-cols': cols } as CSSProperties}>
        {options.map((opt, i) => {
          ...
          return (
            <button
              ...
              onKeyDown={(e: KeyboardEvent) => rovingKeyDown(e, i, options.length, moveTo)}
            >
              <span className="unilens-a11y-choice-check" aria-hidden="true">
                <Icon name="check" size={12} />
              </span>
              {opt.preview?.()}
              <span className="unilens-a11y-choice-label">{opt.label}</span>
              {opt.hint && <span className="unilens-a11y-choice-hint">{opt.hint}</span>}
            </button>
          )
        })}
      </div>
...
export function SwatchPreview({ background, color, sample = 'あA' }: { background: string; color: string; sample?: string }) {
  return (
    <span className="unilens-a11y-swatch" aria-hidden="true" style={{ background, color }}>
      {sample}
    </span>
  )
}
...
export function StripPreview({ filter }: { filter: string }) {
  return <span className="unilens-a11y-strip" aria-hidden="true" style={{ filter }} />
}

/** A text sample showing how a font size/typeface looks. */
export function TextPreview({ sample, style }: { sample: string; style: CSSProperties }) {
  return (
...
export function LinesPreview({ gap }: { gap: number }) {
  return (
    <span className="unilens-a11y-lines" aria-hidden="true" style={{ gap: `${gap}px` }}>
      {[0, 1, 2].map((i) => (
...
function LevelMeter({ level }: { level: number }) {
  ...
  return (
    <span className="unilens-a11y-level-meter" aria-hidden="true">
      {heights.map((h, i) => (
        <span key={h} className={`unilens-a11y-level-bar${i < level ? ' unilens-a11y-level-bar--on' : ''}`} style={{ height: `${h}px` }} />
      ))}
    </span>
  )
}
 ...
 */
export function LevelControl({ label, options, current, focusPrefix, onSelect }: LevelControlProps) {
  const btnRefs = useRef<(HTMLButtonElement | null)[]>([])
  ...
  return (
    <fieldset className="unilens-a11y-group">
      <legend className="unilens-a11y-group-label">{label}</legend>
      <div className="unilens-a11y-level" role="radiogroup" aria-label={label}>
        {options.map((opt, i) => {
          const selected = opt.value === current
          return (
            <button
            ...
            >
              ...
              <span className="unilens-a11y-level-label">{opt.label}</span>
              {opt.hint && <span className="unilens-a11y-level-hint">{opt.hint}</span>}
            </button>
          )
        })}
      </div>
...
export function SwitchRow({ label, hint, checked, focusKey, onChange }: SwitchRowProps) {
  ...
  return (
    <div className="unilens-a11y-switch-row">
      <div className="unilens-a11y-switch-text">
        ...
        {hint && <span className="unilens-a11y-switch-hint">{hint}</span>}
      </div>
      <button
        type="button"
...
export function Stepper({ label, value, decreaseLabel, increaseLabel, focusPrefix, onDecrease, onIncrease }: StepperProps) {
  return (
...
export function Note({ text, tone = 'neutral', iconName }: { text: string; tone?: NoteTone; iconName?: IconName }) {
  return (
