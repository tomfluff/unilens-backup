...
import { DEFAULT_SPEECH_RATE_LEVEL, normalizeSpeechRateLevel } from './speechLevels'
...
export const SETTINGS_DEFAULTS: Settings = { ...A11Y_DEFAULTS, ...AUX_DEFAULTS }
...
function normalizeA11yFields(s: A11ySettings): void {
  ...
  if (s.fontFamily !== 'ud' && s.fontFamily !== 'custom') s.fontFamily = 'standard'
  s.customFontFamily =
    typeof s.customFontFamily === 'string' && s.customFontFamily.trim() !== ''
      ? s.customFontFamily.trim()
      : null
  if (s.fontFamily === 'custom' && !s.customFontFamily) s.fontFamily = 'standard'
...
function normalizeSettings(s: Settings): void {
  ...
  if (s.selectionFontLevel < 1 || s.selectionFontLevel > 3) s.selectionFontLevel = 2
  s.speechRateLevel = normalizeSpeechRateLevel(s.speechRateLevel)
  s.speechHighlight = s.speechHighlight !== false
  if (!['visual', 'text', 'tools'].includes(s.panelTab)) s.panelTab = 'visual'
  if (s.panelLang != null && s.panelLang !== 'ja' && s.panelLang !== 'en') s.panelLang = null
}
...
export const useSettings = create<Settings>()(
  persist(() => ({ ...SETTINGS_DEFAULTS }), {
    ...
    migrate: (persisted: unknown, version: number) => {
      ...
      if (version < 2 && state.theme === 'light') state.theme = 'standard'
...
export function getA11ySettings(): A11ySettings {
  ...
  for (const key of A11Y_SETTING_KEYS) (out as Record<string, unknown>)[key] = s[key]
  return out
}
...
export function patchA11ySettings(patch: Partial<A11ySettings>) {
  ...
  if (next.fontFamily !== undefined && next.fontFamily !== 'custom') next.customFontFamily = null
...
export function resetDisplaySettings() {
  patchA11ySettings({ theme: 'standard', saturation: 'standard', contrast: 'standard' })
}
