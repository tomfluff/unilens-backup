...
import { getAppliedTextScale, refreshAutoTextSize, runAutoTextAction, type TextAnalysis } from './autoTextSize'
import {
...
import { onSettingsChange, saveSettings, settings, type AutoTextMode } from './settings'
import {
  A11Y_LANGS,
  A11Y_LANG_NAMES,
  getA11yLang,
...
import { A11Y_PRESET_IDS, isPresetActive, toggleA11yPreset, type A11yPresetId } from './accessibilityPresets'
import {
  AUTO_MIN_PX_OPTIONS,
  AUTO_MODES,
  CONTRAST_FILTER,
...
function presetMeta(id: A11yPresetId): { label: string; hint: string } {
  switch (id) {
    case 'lowVision':
      return { label: t().presetLowVision, hint: t().presetLowVisionHint }
    case 'colorVision':
      return { label: t().presetColorVision, hint: t().presetColorVisionHint }
    case 'mild':
      return { label: t().presetMild, hint: t().presetMildHint }
    case 'senior':
      return { label: t().presetSenior, hint: t().presetSeniorHint }
    case 'highContrast':
      return { label: t().presetHighContrast, hint: t().presetHighContrastHint }
    case 'focus':
  ...
  return (
    <Card title={t().presetsTitle} iconName="preset" desc={t().presetsDesc}>
      <div className="unilens-a11y-group">
        <span className="unilens-a11y-group-label">{t().presetsGroup}</span>
        <div className="unilens-a11y-choices" style={{ '--da-cols': 2 } as CSSProperties} role="group" aria-label={t().presetsGroup}>
            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  announce(next === 'on' ? t().presetAnnounce(label) : t().presetAnnounceOff(label))
                }}
              >
                {active && (
                  <span className="unilens-a11y-choice-check" aria-hidden="true">
                    <Icon name="check" size={12} />
                  </span>
                )}
                <span className="unilens-a11y-choice-label">{label}</span>
                <span className="unilens-a11y-choice-hint">{active ? t().presetActive : hint}</span>
              </button>
            )
          })}
        </div>
  ...
  return (
    <Card title={t().cuesTitle} iconName="link" desc={t().cuesDesc}>
      <SwitchRow
        ...
        hint={s.linkUnderline ? t().linkUnderlineHintOn : t().linkUnderlineHintOff}
        checked={s.linkUnderline}
        focusKey="link-underline"
        onChange={(checked) => {
          patchA11ySettings({ linkUnderline: checked })
          announce(checked ? t().linkUnderlineOn : t().linkUnderlineOff)
        }}
      />
      <SwitchRow
        label={t().linkBackgroundSwitch}
        hint={s.linkBackground ? t().linkBackgroundHintOn : t().linkBackgroundHintOff}
        checked={s.linkBackground}
        focusKey="link-background"
        onChange={(checked) => {
          patchA11ySettings({ linkBackground: checked })
          announce(checked ? t().linkBackgroundOn : t().linkBackgroundOff)
        }}
      />
      <SwitchRow
        label={t().linkBorderSwitch}
        hint={s.linkBorder ? t().linkBorderHintOn : t().linkBorderHintOff}
      ...
      <SwitchRow
        ...
        hint={s.imageBorder ? t().imageBorderHintOn : t().imageBorderHintOff}
      ...
      <SwitchRow
        ...
        hint={s.elementHighlight ? t().elementHighlightHintOn : t().elementHighlightHintOff}
        checked={s.elementHighlight}
        focusKey="element-highlight"
        onChange={(checked) => {
          patchA11ySettings({ elementHighlight: checked })
          announce(checked ? t().elementHighlightOn : t().elementHighlightOff)
        }}
      />
      <SwitchRow
        label={t().focusEnhanceSwitch}
        hint={s.focusEnhance ? t().focusEnhanceHintOn : t().focusEnhanceHintOff}
        checked={s.focusEnhance}
        focusKey="focus-enhance"
        onChange={(checked) => {
          patchA11ySettings({ focusEnhance: checked })
          announce(checked ? t().focusEnhanceOn : t().focusEnhanceOff)
        }}
      />
      <SwitchRow
        label={t().reduceMotionSwitch}
        hint={s.reduceMotion ? t().reduceMotionHintOn : t().reduceMotionHintOff}
        checked={s.reduceMotion}
        focusKey="reduce-motion"
        onChange={(checked) => {
          patchA11ySettings({ reduceMotion: checked })
          announce(checked ? t().reduceMotionOn : t().reduceMotionOff)
        }}
      />
    </Card>
  )
...
function ColorModeCard() {
  const s = a11ySettings
  return (
    <Card title={t().colorModeTitle} iconName="palette" desc={t().colorModeDesc}>
      <ChoiceGroup<DisplayTheme>
        ...
        options={DISPLAY_THEMES.map((value) => ({
          ...
          preview: () => (
            <SwatchPreview background={THEME_SWATCH[value][0]} color={THEME_SWATCH[value][1]} sample={t().previewSample} />
          ),
        }))}
        onSelect={(value) => {
  ...
  return (
    <Card title={t().toneTitle} iconName="contrast" desc={t().toneDesc}>
      <ChoiceGroup<DisplaySaturation>
        ...
        options={(Object.keys(SATURATION_FILTER) as DisplaySaturation[]).map((value) => ({
          value,
          label: t().saturation[value],
          hint: `${Math.round(SATURATION_FILTER[value] * 100)}%`,
          ariaLabel: t().saturationAria(t().saturation[value]),
          preview: () => <StripPreview filter={`saturate(${SATURATION_FILTER[value]})`} />,
        }))}
        onSelect={(value) => {
      ...
      <ChoiceGroup<DisplayContrast>
        ...
        current={s.contrast}
        options={(Object.keys(CONTRAST_FILTER) as DisplayContrast[]).map((value) => ({
          value,
          label: t().contrast[value],
          hint: `${Math.round(CONTRAST_FILTER[value] * 100)}%`,
          ariaLabel: t().contrastAria(t().contrast[value]),
          preview: () => <StripPreview filter={`contrast(${CONTRAST_FILTER[value]})`} />,
        }))}
        onSelect={(value) => {
    ...
    <Card title={t().pageTextTitle} iconName="type" desc={t().pageTextDesc}>
      <ChoiceGroup<A11yFontSize>
        ...
        options={sizes.map((value, i) => ({
          ...
          hint: sizeHints[i],
          ariaLabel: t().fontSizeAria(t().fontSize[value], sizeHints[i]),
          preview: () => <TextPreview sample="Aa" style={{ fontSize: `${PREVIEW_FONT_PX[i]}px`, fontWeight: 700 }} />,
        }))}
        onSelect={(value) => {
        ...
        options={[
          {
            ...
            preview: () => (
              <TextPreview
                ...
                style={{ fontSize: '17px', fontWeight: 700, fontFamily: STANDARD_FONT_STACK }}
              />
            ),
          },
          {
            ...
            preview: () => (
              <TextPreview sample={t().previewSample} style={{ fontSize: '17px', fontWeight: 700, fontFamily: UD_FONT_STACK }} />
            ),
          },
        ]}
        onSelect={(value) => {
          patchA11ySettings({ fontFamily: value })
          announce(t().typefaceAnnounce(value === 'ud' ? t().typefaceUd : t().typefaceStandard))
        }}
      />
      <ChoiceGroup<A11yLineHeight>
        ...
        onSelect={(value) => {
          ...
          announce(t().lineHeightAnnounce(value === 'wide' ? t().lineHeightWide : t().lineHeightStandard))
        }}
      />
        ...
        options={[
          {
            ...
            hint: '0',
            ariaLabel: t().letterSpacingAria(t().letterSpacingStandard),
            preview: () => (
              <TextPreview sample={t().previewSample} style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '0' }} />
            ),
          },
          {
            ...
            preview: () => (
              <TextPreview sample={t().previewSample} style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '0.12em' }} />
            ),
          },
        ]}
        onSelect={(value) => {
          patchA11ySettings({ letterSpacing: value })
          announce(t().letterSpacingAnnounce(value === 'wide' ? t().letterSpacingWide : t().letterSpacingStandard))
        }}
      />
    </Card>
  )
    ...
    >
      <LevelControl
        ...
        options={TEXT_ADJUST_LEVELS.map((value) => ({
          ...
          ariaLabel: value === 0 ? t().bodyExpandAriaOff : t().bodyExpandAria(labels[value].join(' ')),
        }))}
        onSelect={(value) => {
          patchA11ySettings({ bodyTextExpandLevel: value as TextAdjustLevel })
          ...
          else announce(t().bodyExpandAnnounce(labels[value as TextAdjustLevel][0], getLastBodyTextScan().expanded))
        }}
      />
      {level > 0 && (
        <>
          <Note text={t().bodyExpandNote(scan.expanded, scan.scanned)} tone="info" iconName="check" />
          <ButtonRow>
            <Button
              label={t().rescan}
              iconName="refresh"
              ...
              onClick={() => {
                ...
                announce(t().rescanBodyResult(result.expanded, result.scanned))
                setBump((n) => n + 1)
              }}
            />
          </ButtonRow>
    ...
    >
      <LevelControl
        ...
        options={TEXT_ADJUST_LEVELS.map((value) => ({
          ...
          label: labels[value][0],
          hint: labels[value][1] || undefined,
          ariaLabel: value === 0 ? t().smallBoostAriaOff : t().smallBoostAria(labels[value].join(' ')),
        }))}
        onSelect={(value) => {
          patchA11ySettings({ smallTextBoostLevel: value as TextAdjustLevel })
          if (value === 0) announce(t().smallBoostAnnounceOff)
          else announce(t().smallBoostAnnounce(labels[value as TextAdjustLevel][0], getLastSmallTextScan().boosted))
        }}
      />
      {level > 0 && (
        <>
          <Note text={t().smallBoostNote(scan.boosted, scan.scanned)} tone="info" iconName="check" />
          <ButtonRow>
            <Button
              label={t().rescan}
              iconName="refresh"
              ...
              onClick={() => {
                ...
                announce(t().rescanSmallResult(result.boosted, result.scanned))
                setBump((n) => n + 1)
              }}
            />
          </ButtonRow>
...
function SelectionCard({
  ...
  const detail = info.hasSelection
    ? [
        ...
        info.appliedScale ? `${Math.round(info.appliedScale * 100)}%` : null,
  ...
  return (
    <Card title={t().selectionTitle} iconName="selection" desc={t().selectionDesc}>
      {info.hasSelection ? (
        <Note text={detail as string} tone="info" iconName="selection" />
      ) : (
        <Note text={t().selectionEmpty} tone="neutral" iconName="selection" />
      )}
      ...
      <ChoiceGroup<string>
        ...
        options={SELECTION_LEVELS.map((value) => ({
          ...
          preview: () => <TextPreview sample="Aa" style={{ fontSize: `${PREVIEW_FONT_PX[value - 1]}px`, fontWeight: 700 }} />,
        }))}
        onSelect={(value) => {
          const result = applySelectionFontLevel(Number(value) as SelectionFontLevel)
      ...
      {lastMessage && <Note text={lastMessage.text} tone={lastMessage.ok ? 'ok' : 'error'} />}
    </Card>
  )
}
...
function AutoTextCard({
  ...
  return (
    <Card title={t().autoTitle} iconName="auto" desc={t().autoDesc} badge={enabled ? `×${scale.toFixed(2)}` : undefined}>
      <SwitchRow
        label={t().autoSwitch}
        ...
        onChange={(checked) => {
          ...
          announce(checked ? t().autoOnAnnounce(scaleText()) : t().autoOffAnnounce)
        }}
      />
      ...
      {enabled && (
        <>
          <ChoiceGroup<Exclude<AutoTextMode, 'off'>>
            ...
            current={settings.autoTextMode as Exclude<AutoTextMode, 'off'>}
            options={AUTO_MODES.map((value) => ({
              value,
              label: t().autoMode[value].label,
              hint: t().autoMode[value].hint,
              ariaLabel: t().autoModeAria(t().autoMode[value].label, t().autoMode[value].hint),
            }))}
            onSelect={(value) => {
              settings.autoTextMode = value
              saveSettings()
              setLastAnalysis(refreshAutoTextSize())
              announce(t().autoModeAnnounce(t().autoMode[value].label, scaleText()))
            }}
          />
          ...
          <ChoiceGroup<string>
            ...
            onSelect={(value) => {
              ...
              saveSettings()
              setLastAnalysis(refreshAutoTextSize())
              announce(t().autoMinAnnounce(Number(value), scaleText()))
            }}
          />
          ...
          <ButtonRow>
            <Button
              ...
              onClick={() => {
                ...
                setLastAnalysis(a)
                announce(a ? t().analyzeAnnounce(a.medianPx, Math.round(a.smallTextRatio * 100), scaleText()) : t().analyzeEmpty)
              }}
            />
          </ButtonRow>
          ...
          {lastAnalysis && (
            <Metrics
              rows={[
                [t().metricSamples, t().metricSamplesValue(lastAnalysis.sampleCount)],
                [t().metricMedian, `${lastAnalysis.medianPx}px`],
                [t().metricMinMax, `${lastAnalysis.minPx} / ${lastAnalysis.maxPx}px`],
                [t().metricSmallRatio, `${Math.round(lastAnalysis.smallTextRatio * 100)}%`],
...
export function ToolsTab({ lastAnalysis, setLastAnalysis, lastSelectionMessage, setLastSelectionMessage }: ToolsTabProps) {
  useExternalSignal(onSettingsChange)
  ...
  return (
    <>
      <SelectionCard lastMessage={lastSelectionMessage} setLastMessage={setLastSelectionMessage} />
      {/* <SpeakCard /> */}
      <AutoTextCard lastAnalysis={lastAnalysis} setLastAnalysis={setLastAnalysis} />
    </>
  )
}
