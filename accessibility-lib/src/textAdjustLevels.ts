...
export const BODY_TEXT_EXPAND_CONFIG: Record<1 | 2 | 3, BodyTextExpandLevelConfig> = {
  1: { scale: 1.15, minPx: 16 },
  2: { scale: 1.25, minPx: 16 },
  3: { scale: 1.35, minPx: 16 },
}

export const SMALL_TEXT_BOOST_CONFIG: Record<1 | 2 | 3, SmallTextBoostLevelConfig> = {
  1: { minPx: 14, addPx: 2 },
  2: { minPx: 16, addPx: 3 },
  3: { minPx: 18, addPx: 4 },
}
...
export function bodyTextExpandConfig(level: TextAdjustLevel): BodyTextExpandLevelConfig | null {
  if (level === 1 || level === 2 || level === 3) return BODY_TEXT_EXPAND_CONFIG[level]
  return null
}

export function smallTextBoostConfig(level: TextAdjustLevel): SmallTextBoostLevelConfig | null {
  if (level === 1 || level === 2 || level === 3) return SMALL_TEXT_BOOST_CONFIG[level]
  return null
}
...
export function computeBodyTextTargetPx(originalPx: number, level: TextAdjustLevel): number {
  const cfg = bodyTextExpandConfig(level)
  if (!cfg) return originalPx
  return Math.max(cfg.minPx, Math.round(originalPx * cfg.scale * 100) / 100)
}

export function computeSmallTextTargetPx(originalPx: number, level: TextAdjustLevel): number {
  const cfg = smallTextBoostConfig(level)
  if (!cfg) return originalPx
  return Math.max(cfg.minPx, Math.round((originalPx + cfg.addPx) * 100) / 100)
}
