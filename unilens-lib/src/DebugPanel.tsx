...
const rowStyle: CSSProperties = { font: '11px monospace', color: '#cdE', lineHeight: 1.6, whiteSpace: 'pre-wrap' }

function Section({ title, children }: { title: string; children?: ReactNode }) {
  return (
    <>
      <div style={{ color: CYAN, font: '700 11px sans-serif', margin: '10px 0 4px', letterSpacing: 0.5 }}>
        {title.toUpperCase()}
      </div>
      {children}
    </>
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ color: CYAN, font: '700 13px sans-serif' }}>UniLens debug</span>
        <button
          style={{ background: 'none', border: 'none', color: '#889', cursor: 'pointer', fontSize: 14 }}
      ...
      <Section title="Pointer trace">
        <canvas
          ...
          width={264}
          height={66}
          style={{ borderRadius: 6, background: '#101020', display: 'block' }}
        />
        <div style={rowStyle}>
          {last
      ...
      <Section title="Dwell detector">
        <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 4, height: 8, margin: '2px 0 4px' }}>
          <div
            style={{
      ...
      <Section title="Backend">
        <div style={rowStyle}>{`${sources.backend() || '(same origin)'}\n${health}`}</div>
      </Section>
    </div>
  )
}
