 ...
 */
function homeOnAxis(pos: number, size: number, layoutExtent: number, viewportExtent: number, docResolved: boolean) {
  const offDocumentEdge = docResolved && layoutExtent > viewportExtent && pos > viewportExtent - size
...
function prunePins() {
  pins = pins.filter((p) => {
    const live =
      p.el.isConnected && getComputedStyle(p.el).position === 'fixed' && (p.el.offsetWidth || p.el.offsetHeight)
    if (!live) {
...
export function stripFixedPins(doc: Document) {
  ...
  for (const el of doc.querySelectorAll<HTMLElement>('[data-unilens-sticky]')) {
    el.style.position = ''
...
export function getView(): { x: number; y: number } {
  return frozen ? { x: pan.x, y: pan.y } : { x: window.scrollX, y: window.scrollY }
}

/** client coords -> content (layout) coords, correct under either engine */
export function clientToContent(clientX: number, clientY: number): { x: number; y: number } {
  const v = getView()
...
export function setView(x: number, y: number) {
  if (frozen) {
    pan.x = Math.max(0, Math.min(x, Math.max(0, layoutW * scale - window.innerWidth)))
    pan.y = Math.max(0, Math.min(y, Math.max(0, layoutH * scale - window.innerHeight)))
    paint()
    applyPins()
  } else {
    window.scrollTo(x, y)
...
function paint() {
  ...
  else if (frozen) document.body.style.transform = `translate(${-pan.x}px, ${-pan.y}px) scale(${scale})`
...
function onDblClick(e: MouseEvent) {
  ...
  const { x: cx, y: cy } = clientToContent(rect.left + rect.width / 2, rect.top + rect.height / 2)
...
function wheelHandledElsewhere(e: WheelEvent): boolean {
  ...
  for (let el: Element | null = target; el && el !== document.body; el = el.parentElement) {
    ...
    if (SCROLLABLE.test(style.overflowY) && el.scrollHeight > el.clientHeight) {
      if (e.deltaY < 0 ? el.scrollTop > 0 : Math.ceil(el.scrollTop + el.clientHeight) < el.scrollHeight) return true
    }
    if (SCROLLABLE.test(style.overflowX) && el.scrollWidth > el.clientWidth) {
      if (e.deltaX < 0 ? el.scrollLeft > 0 : Math.ceil(el.scrollLeft + el.clientWidth) < el.scrollWidth) return true
    }
  }
  return false
}
...
function onPanKey(e: KeyboardEvent) {
  ...
  if (t && (t.isContentEditable || /^(INPUT|TEXTAREA|SELECT)$/.test(t.tagName) || t.closest?.('#unilens-root'))) return
  const v = getView()
