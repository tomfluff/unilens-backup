...
function evaluate(): DwellDebug {
    ...
    // the root stays on the page, and so does a hidden chat: a chat that shows is open
    if (
        document
            .getElementById("unilens-root")
            ?.firstElementChild?.getClientRects().length
    )
    ...
    if (now - pts[pts.length - 1].t > 2000)
        return { ...d, blocked: "pointer idle" };

    const cx = pts.reduce((s, p) => s + p.x, 0) / pts.length;
    const cy = pts.reduce((s, p) => s + p.y, 0) / pts.length;
    d.centroid = { x: Math.round(cx), y: Math.round(cy) };
    d.spreadPx = Math.round(
        Math.max(...pts.map((p) => Math.hypot(p.x - cx, p.y - cy))),
    );
